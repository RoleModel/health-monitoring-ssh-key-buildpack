...
- Encode the private key as a base64 string and add it as the `HEALTH_MONITORING_SSH_KEY` environment variable of the heroku app.

- Make a comma separated list of the hosts for which the ssh key should be used and add it as the `HEALTH_MONITORING_SSH_KEY_HOSTS` environment variable of the heroku app.

  ```
  # OSX
  $ heroku config:set HEALTH_MONITORING_SSH_KEY=$(base64 --input ~/.ssh/deploy_key) HEALTH_MONITORING_SSH_KEY_HOSTS=bitbucket.org,github.com

  # Linux
  $ heroku config:set HEALTH_MONITORING_SSH_KEY=$(base64 ~/.ssh/deploy_key) HEALTH_MONITORING_SSH_KEY_HOSTS=bitbucket.org,github.com
  ```
